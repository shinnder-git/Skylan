...
class SkylanApp(MDApp):
	...
	def on_start(self):
		users = get_users()
		name = os.environ.get('USERNAME')
		self.root.get_screen('main').ids.spam_text.text = users[str(name)]['text']
...
def open_account():
	...
	if str(name) in users:
		...
		users[str(name)]['text'] = "Сюда вы сможете написать любой текст, который хотите заспамить в чате..."
